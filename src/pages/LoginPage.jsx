...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // 팝업 로그인은 성공했지만 Cross-Origin-Opener-Policy로 팝업 뒷정리(window.close)가
      // 막히면서 에러가 날 수 있음 — 실제 로그인 여부(auth.currentUser)로 성공/실패를 판단한다.
      if (auth.currentUser) {
        try { await saveGoogleProfile(auth.currentUser); } catch { /* 프로필 저장 실패는 무시 */ }
        navigate('/');
